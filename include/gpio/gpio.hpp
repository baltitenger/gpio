...
struct LineHandle {
	...
	constexpr LineHandle(const Offsets &offsets, std::string_view consumer,
			Dir dir, uint flags = 0, uint64_t defaults = -1) {
		if (offsets.size() > MAX) {
			throw std::runtime_error(
				"Maximum number of requested lines exceeded!");
...
struct SetLineValues {
	...
	constexpr void *data() noexcept { return &data_; }
	SetLineValues(uint64_t values, offset_t count = MAX) noexcept {
...
struct EventHandle {
	...
	constexpr void *data() noexcept { return &data_; }
	template <typename Offsets = std::initializer_list<offset_t>>
	constexpr EventHandle(offset_t offset, std::string_view consumer,
			uint flags = 0, Edge events = Both) {
...
struct Chip {
	...
	Chip(Ioc &ioc, std::string_view devName)
		: Chip(ioc, {ioc, open(std::string("/dev/").append(devName).c_str(), 0)}) {}

	Chip(Ioc &ioc, Fd &&fd)
		: Chip(ioc, std::move(fd), ioctl::ioctl(fd, ioctl::ChipInfo())) {}

	Chip(Ioc &ioc, Fd &&fd, const gpiochip_info &info)
		: ioc(ioc), fd(std::move(fd)), name(info.name), label(info.label)
		, numLines(static_cast<offset_t>(info.lines)) {}
